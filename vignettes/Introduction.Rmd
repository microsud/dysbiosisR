...
head(all.dysbiosis)
```
The different dysbiosis score can be investigated in the context of specific host features.  

```{r fig.width=5, fig.height=3, fig.align='center'}
# Age
p1 <- cbind(all.dysbiosis,
      dysbiosis_1[c("age","BMI", "disease")]) |> 
  ggplot(aes(age,CentroidDist_score,color=disease)) +
  geom_point() +
  theme_bw(base_size = 12) +
  geom_smooth() +
  scale_color_manual("Group", values = c(CRC="brown3", healthy="steelblue"))+
  labs(y="Dysbiosis Score (Centroid)")
p1
```

```{r fig.width=5, fig.height=3, fig.align='center'}
# BMI  
p2 <- cbind(all.dysbiosis,
      dysbiosis_1[c("age","BMI", "disease")]) |> 
  ggplot(aes(BMI,CentroidDist_score,color=disease)) +
  geom_point() +
  theme_bw(base_size = 12) +
  geom_smooth()+
  scale_color_manual("Group", values = c(CRC="brown3", healthy="steelblue")) +
